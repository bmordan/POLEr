...
module.exports = {
    all: async () => {
        return await openSessionAndRunQuery('MATCH (n)-[r]-() RETURN r,n;')
    },
    personByName: async name => {
        return await openSessionAndRunQuery(`MATCH (p:PERSON)-[r]-(n) WHERE p.name="${name}" RETURN p,r,n;`)
    },
    expandNodeById: async id => {
        return await openSessionAndRunQuery(`MATCH (t)-[r]-(n) WHERE id(t)=${id} RETURN r,n;`)
    }
}
